...
using System.Data;
using System.IO; // Trigger dotnet watch clean rebuild
using System.Linq;
...
namespace ticketing_system_backend.Controllers
{
  ...
  public class ManagerController : ControllerBase
  {
    ...
    public ManagerController(ILogger<ManagerController> logger, IManager managerRepo, ITicketRepository ticketRepository, IEmailService emailService, IWebHostEnvironment webHostEnvironment, INotificationService notificationService)
    {
      _logger = logger;
    ...
    public async Task<IActionResult> GetMyCustomers()
    {
      ...
      if (roleClaim == "Assignee" || roleClaim == "Executive" || roleClaim == "Developer")
      {
        var assignedCustomers = await _managerRepo.GetCustomersByAssigneeIdAsync(userId);
        return Ok(assignedCustomers);
      }

      IEnumerable<int> customerIds;
      if (roleClaim == "PM" || roleClaim == "Project Manager")
      {
      ...
      var filtered = allCustomersList.Where(c => customerIds.Contains(c.Id));
      return Ok(filtered);
    }
    ...
    // --- TICKET MANAGEMENT ---
    [Authorize(Roles = "SuperManager,Manager,PM,Super Admin,SuperAdmin,Admin,HOD,Assignee")]
    [HttpGet("tickets")]
       ...
       [FromQuery] int pageSize = 100000)
    {
      try
      {
        ...
        var response = await _managerRepo.GetAllTicketsAsync(
            status, priority, customerId, assignedToId, createdByUserId, dateFrom, dateTo, pageNumber, pageSize, userId, userRole);
        return Ok(response);
      }
      catch (Exception ex)
      {
    ...
    public async Task<IActionResult> UpdateAssignment(int id, [FromBody] UpdateAssignmentRequest request)
    {
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while updating assignment." });
      }
    }
    ...
    public async Task<IActionResult> UpdateTicketByPm(int ticketId, int pmId, [FromBody] PmTicketUpdateRequest request)
    {
      try 
      {
          ...
          try
          {
                ...
                !newStatus.Equals(ticketBeforeUpdate.Status ?? "", StringComparison.OrdinalIgnoreCase))
            {
              try
              {
                if (customerEmail != null)
                {
                  var emailContent = EmailTemplates.TicketStatusUpdate(customerFullName!, ticketNumberStr, newStatus, pmFullName, ticketSubjectStr, isCustomer: true, ticketId: ticketId, remark: request.Note);
        ...
        return Ok(new { success = true, message = "Ticket updated successfully by PM." });
      }
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while updating ticket." });
      }
    }
    ...
    public async Task<IActionResult> UpdateTicketByCustomer(int ticketId, int customerId, [FromBody] CustomerUpdateTicketRequest request)
    {
      try
      {
        ...
        try
        {
          customer = await _managerRepo.GetUserByIdAsync(customerId, "Customer"); 

          int? assignedToId = null;
          if (!string.IsNullOrEmpty(ticketBeforeUpdate.AssignedTo) && int.TryParse(ticketBeforeUpdate.AssignedTo, out var parsedAssigneeId))
          {
        ...
        {
          _logger.LogError(ex, "Failed to fetch user/assignee details before background email task for ticket {TicketId}", ticketId);
        }
        ...
        var sourceEmail = ticketBeforeUpdate.SourceEmail;
          ...
          try
          {
            ...
            if (!string.IsNullOrEmpty(newStatus) && newStatus != ticketStatus)
            {
               if (!string.IsNullOrWhiteSpace(assigneeEmail))
               {
                 try
                 {
                   var emailContent = EmailTemplates.TicketStatusUpdate(assigneeFullName!, ticketNumber, newStatus, customerFullName ?? "the customer", ticketBeforeUpdate.Subject, isCustomer: false, ticketId: ticketId, remark: request.Note ?? request.Remark);
                   await _emailService.SendEmailAsync(assigneeEmail, emailContent.Subject, emailContent.Body, threadId);
                 }
                 catch (Exception ex) { _logger.LogError(ex, "Failed to send Status update email to Assignee {AssigneeEmail}", assigneeEmail); }
               }
               if (!string.IsNullOrWhiteSpace(customerEmail))
               {
                 try
                 {
                   var emailContent = EmailTemplates.TicketStatusUpdate(customerFullName!, ticketNumber, newStatus, customerFullName!, ticketBeforeUpdate.Subject, isCustomer: true, ticketId: ticketId, remark: request.Note ?? request.Remark);
                   await _emailService.SendEmailAsync(customerEmail, emailContent.Subject, emailContent.Body, threadId);
                 }
                 catch (Exception ex) { _logger.LogError(ex, "Failed to send Status update email to Customer {CustomerEmail}", customerEmail); }
               }
               if (!string.IsNullOrWhiteSpace(sourceEmail))
               {
                 try
                 {
                   var emailContent = EmailTemplates.TicketStatusUpdate(customerFullName ?? "Customer", ticketNumber, newStatus, customerFullName ?? "the customer", ticketBeforeUpdate.Subject, isCustomer: true, ticketId: ticketId, remark: request.Note ?? request.Remark);
                   await _emailService.SendEmailAsync(sourceEmail, emailContent.Subject, emailContent.Body, threadId);
                 }
                 catch (Exception ex) { _logger.LogError(ex, "Failed to send Status update email to SourceEmail {SourceEmail}", sourceEmail); }
               }
            }
          }
          catch (Exception ex)
        ...
        return Ok(new { success = true, message = "Ticket updated successfully by customer." });
      }
      catch (InvalidOperationException ex)
      {
    ...
    [Authorize(Roles = "Assignee,SuperManager,Manager,Super Admin,HOD,PM,Project Manager")]
    ...
    public async Task<IActionResult> SubmitForReview(int id, int developerId)
    {
      try
      {
        ...
        try
        {
          customer = await _managerRepo.GetUserByIdAsync(ticket.CustomerId, "Customer"); 
          developer = await _managerRepo.GetAssigneeByIdAsync(developerId); 
        }
        catch (Exception ex)
        {
          _logger.LogError(ex, "Failed to fetch customer/developer details before review submission email task for ticket {TicketId}", id);
        }

        var reviewCustEmail = customer?.Email;
        var reviewCustName = customer?.FullName;
        var developerName = developer?.FullName ?? $"Developer ID {developerId}";
        var reviewSourceEmail = ticket.SourceEmail;
        ...
        _ = Task.Run(async () =>
        {
          try
          {
            ...
            if (!string.IsNullOrWhiteSpace(reviewCustEmail))
            {
               try
               {
                 var emailContent = EmailTemplates.TicketStatusUpdate(reviewCustName!, ticketNumber, "In Review", developerName, ticketSubject, isCustomer: true, ticketId: id);
                 await _emailService.SendEmailAsync(reviewCustEmail, emailContent.Subject, emailContent.Body, threadId);
               }
               catch (Exception ex) { _logger.LogError(ex, "Failed to send Review email to Customer {CustomerEmail}", reviewCustEmail); }
            }
            if (!string.IsNullOrWhiteSpace(reviewSourceEmail))
            {
               try
               {
                 var emailContent = EmailTemplates.TicketStatusUpdate(reviewCustName ?? "Customer", ticketNumber, "In Review", developerName, ticketSubject, isCustomer: true, ticketId: id);
                 await _emailService.SendEmailAsync(reviewSourceEmail, emailContent.Subject, emailContent.Body, threadId);
               }
               catch (Exception ex) { _logger.LogError(ex, "Failed to send Review email to SourceEmail {SourceEmail}", reviewSourceEmail); }
            }
          }
          catch (Exception ex)
          {
        ...
        return Ok(new { success = true, message = "Ticket submitted for review successfully." });
      }
      catch (Exception ex)
      {
        _logger.LogError(ex, "Error submitting ticket {TicketId} for review by developer {DeveloperId}.", id, developerId);
        return StatusCode(500, new { success = false, message = "An internal server error occurred while submitting for review." });
      }
    }

    [Authorize(Roles = "SuperManager,Manager,Super Admin,HOD,Developer,Assignee,PM")]
    ...
    public async Task<IActionResult> UpdateStatus(int id, [FromBody] UpdateStatusRequest request)
    {
      try
      {
        ...
        if (ticketBeforeUpdate != null && !request.Status.Equals(ticketBeforeUpdate.Status ?? "", StringComparison.OrdinalIgnoreCase))
        {
            var customer = await _managerRepo.GetUserByIdAsync(ticketBeforeUpdate.CustomerId, "Customer");
            var customerFullName = customer?.FullName;
            var customerEmail = customer?.Email;
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (customerEmail != null)
                    {
                        var emailContent = EmailTemplates.TicketStatusUpdate(customerFullName!, ticketNumberStr, newStatus, "The Support Team", ticketSubjectStr, isCustomer: true, ticketId: id, remark: request.Note);
                        await _emailService.SendEmailAsync(customerEmail, emailContent.Subject, emailContent.Body, threadId);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Background email error (Status Update) for ticket {TicketId}", id);
                    System.IO.File.WriteAllText(@"C:\Users\USER\Desktop\email_error_dev.txt", ex.ToString());
                }
            });
        }
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while updating status." });
      }
    }

    [Authorize(Roles = "SuperManager,Manager,Super Admin,HOD,PM,Project Manager,Assignee,Executive,Developer")]
    ...
    public async Task<IActionResult> SetTicketDeadline(int ticketId, [FromBody] SetDeadlineRequest request)
    {
      ...
      try
      {
        ...
        return Ok(new { success = true, message = "Ticket deadline set successfully." });
      }
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while setting deadline." });
      }
    }

    [Authorize(Roles = "SuperManager,Manager,Super Admin,HOD,Assignee,Executive,Developer,Customer,PM,Project Manager")]
    ...
    {
      if (request.Role != "Customer" && request.Role != "PM" && request.Role != "SuperManager" && request.Role != "Super Admin") return BadRequest("Invalid user role specified.");
      ...
      try 
      {
        ...
        if (newUser != null && newUser.Role == "Customer")
        {
          ...
          if (request.DefaultAssigneeId.HasValue)
          {
            var assignee = await _managerRepo.GetAssigneeByIdAsync(request.DefaultAssigneeId.Value);
    ...
    [Authorize(Roles = "SuperManager,Super Admin,PM,Project Manager,Manager,Assignee,Executive,Developer,HOD")]
    ...
    public async Task<IActionResult> UpdateUser(int id, [FromBody] UpdateUserRequest request)
    {
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while updating user." });
      }
    }
    ...
    public async Task<IActionResult> DeleteUser(int id)
    {
      try
      {
        var user = await _managerRepo.GetUserByIdAsync(id, "Customer");
        if (user == null)
        {
    ...
    public async Task<IActionResult> UpdatePm(int id, [FromBody] UpdatePmRequest request)
    {
      try
      {
        ...
        return Ok(new { success = true, message = "PM details updated successfully." });
      }
      catch (SqlException ex) when (ex.Number == 50000) 
      {
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while updating PM." });
      }
    }
    ...
    {
      var assignees = await _managerRepo.GetAllAssigneesAsync(null);
      return Ok(assignees);
    }
    ...
    public async Task<IActionResult> UpdateAssignee(int id, [FromBody] UpdateAssigneeRequest request)
    {
      try 
      {
        ...
        return Ok(new { success = true, message = "Assignee updated successfully." });
      }
      catch (SqlException ex) when (ex.Number == 50000) 
      {
      ...
      catch (Exception ex)
      {
        ...
        return StatusCode(500, new { success = false, message = "An internal server error occurred while updating assignee." });
      }
    }
    ...
    public async Task<IActionResult> GetCommunications(int ticketId)
    {
      try
      {
        ...
        // If user is a Customer, sanitize and remove private internal notes and developer channels
        if (User.IsInRole("Customer"))
        {
          response.GlobalNotes = new List<TicketCommunication>();
          response.DeveloperChannel = new List<TicketCommunication>();
          response.UpdateNotesDeveloper = new List<TicketCommunication>();
        }
    ...
    public async Task<IActionResult> AddCommunication(int ticketId, [FromForm] NewCommentRequest newComment, [FromForm] List<IFormFile> files)
    {
      ...
      // If user is in role Customer, force channel to Customer
      if (User.IsInRole("Customer") || string.Equals(newComment.PostedByUserRole, "Customer", StringComparison.OrdinalIgnoreCase))
      {
      ...
      if (string.IsNullOrWhiteSpace(newComment.CommentText) && (files == null || files.Count == 0)) return BadRequest("Cannot post an empty comment without attachments.");
    ...
    public async Task<IActionResult> CreateCustomStatus([FromBody] CustomStatusRequest request)
    {
      try
      {
        ...
        var newStatus = await _managerRepo.CreateCustomStatusAsync(request.StatusName);
        return Ok(newStatus);
      }
      catch (InvalidOperationException ex)
      {
    ...
    public async Task<IActionResult> CreatePriority([FromBody] PriorityRequest request)
    {
      try
      {
        if (request == null || string.IsNullOrWhiteSpace(request.PriorityName) || request.TATHours <= 0)
          return BadRequest("Priority name and valid TAT hours are required.");
          
        var newPriority = await _managerRepo.CreatePriorityAsync(request.PriorityName, request.TATHours, request.ResponseSLA);
...
// trigger dotnet watch fresh rebuild
