...
namespace ticketing_system_backend.Controllers
{
  ...
  public class AuthController : ControllerBase
  {
      ...
      public async Task<IActionResult> UnifiedLogin([FromBody] LoginRequest request)
      {
          ...
          var user = await _connection.QueryFirstOrDefaultAsync<User>(customerSql, new { request.UserNumber });
          ...
          if (user == null || user.Password == null || request.Password != user.Password)
          {
              var staffSql = $@"
                  SELECT 
                      CASE 
                      WHEN u.UserTypeId = 4 THEN u.UserTypeMapId 
                      ELSE u.UserId 
                  END AS Id,
                  ...
                  WHERE u.UserName = @UserNumber AND u.IsActive = 1";

              var staff = await _connection.QueryFirstOrDefaultAsync<User>(staffSql, new { request.UserNumber });
              if (staff == null || staff.Password == null || request.Password != staff.Password)
              {
          ...
          var overrides = await _connection.QueryAsync<(string PermissionKey, bool IsEnabled)>(
              ...
              new { UserId = user.Id, UserRole = user.Role });
          ...
          var detailViewType = await _connection.QueryFirstOrDefaultAsync<string>(
              ...
              new { UserId = user.Id, UserRole = user.Role }) ?? "Default";
      ...
      public async Task<IActionResult> RefreshSession()
      {
          ...
          var overrides = await _connection.QueryAsync<(string PermissionKey, bool IsEnabled)>(
              "SELECT PermissionKey, IsEnabled FROM dbo.UserPermissions WHERE UserId = @UserId AND UserRole = @UserRole",
              new { UserId = userId, UserRole = role });
          ...
          var detailViewType = await _connection.QueryFirstOrDefaultAsync<string>(
              "SELECT DetailViewType FROM dbo.UserPreferences WHERE UserId = @UserId AND UserRole = @UserRole",
              new { UserId = userId, UserRole = role }) ?? "Default";
      ...
      public async Task<IActionResult> Refresh()
      {
          ...
          if (stored.UserRole == "Customer")
          {
              userNumber = await _connection.QueryFirstOrDefaultAsync<string>(
                  $"SELECT CustomerCode FROM {DbConfig.CJDarclDb}.dbo.CL_Master_Customer WHERE CustomerId = @UserId AND IsActive = 1",
                  new { UserId = stored.UserId });

              if (string.IsNullOrEmpty(userNumber))
              {
                  userNumber = await _connection.QueryFirstOrDefaultAsync<string>(
                      $"SELECT UserName FROM {DbConfig.CJDarclDb}.dbo.CL_Master_User WHERE UserId = @UserId",
                      new { UserId = stored.UserId });
              }
          }
          else
