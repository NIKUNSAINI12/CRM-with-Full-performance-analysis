...
DbConfig.CJDarclDb = builder.Configuration["CJDarclDbName"] ?? "CJDarcl";
DbConfig.TicketingDb = builder.Configuration["TicketingDbName"] ?? "CRMDarcl";
...
builder.Services.AddScoped<IPerformanceAnalyticsRepository, PerformanceAnalyticsRepository>();
...
builder.Services.AddSingleton<IEmailService>(provider =>
{
    ...
    var email = config.GetSection("EmailSettings");
    ...
    var company = config["EmailCompany"] ?? email["EmailCompany"] ?? "Empower Logics";
    return new SmtpEmailService(host, port, from, pwd, company);
});
...
// ═══════════════════════════════════════════════════════════════════════
// Configure EmailTemplates static settings
ticketing_system_backend.Helpers.EmailTemplates.BaseUrl = builder.Configuration["FrontendBaseUrl"] ?? "http://localhost:4200";
ticketing_system_backend.Helpers.EmailTemplates.ApiBaseUrl = builder.Configuration["ApiBaseUrl"] ?? "http://localhost:5166";
ticketing_system_backend.Helpers.EmailTemplates.CompanyName = builder.Configuration["EmailCompany"] 
                                                              ?? builder.Configuration["EmailSettings:EmailCompany"] 
                                                              ?? "Empower Logics";
