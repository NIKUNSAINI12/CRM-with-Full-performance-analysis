...
export class App implements OnInit {
  ...
  public globalSearchType: 'ticket' | 'docket' | 'auto' = 'auto';
  ...
  public async performGlobalSearch(): Promise<void> {
    ...
    try {
      ...
      console.error('[HEADER SEARCH] Global search failed:', e);
  ...
  public openSearchResult(item: any): void {
    this.showGlobalSearchModal = false;
    if (item && item.id) {
      this.router.navigate(['/customer/ticket', item.id]);
    }
  }
}
