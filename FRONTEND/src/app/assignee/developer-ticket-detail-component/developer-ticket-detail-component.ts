...
export class DeveloperTicketDetailComponent implements OnInit {
  ...
  public customerNotes: TicketCommunicationDto[] = [];           // ADDED: Customer chat notes
  public allDeveloperComms: TicketCommunicationDto[] = []; // NEW: Combined & sorted list
  public activeTab: 'developer' | 'customerChat' | 'timeline' | 'relations' = 'developer';
  ...
  private loadTicketDetails(ticketId: number): void {
    ...
    Promise.all([
    ...
    ]).then(([ticketData, productsData, allTicketsResponse, historyData, deadlineData, commsResponse, timelineAnalysisData, devStatusesData]) => {
      ...
      if (devStatusesData && devStatusesData.length > 0) {
        this.devStatusOptions = devStatusesData.filter((s: any) => s.isActive).map((s: any) => s.statusName);
      } else {
        this.devStatusOptions = ['Open', 'In Progress', 'Work Done', 'Pending', 'Resolved', 'Closed', 'On Hold'];
      }
      if (this.ticket) {
      ...
      const custChannel = appendIstOffset(commsResponse.customerChannel || []);
      const custNotes = appendIstOffset(commsResponse.updateNotesCustomer || []);
      this.customerNotes = [...custChannel, ...custNotes]
  ...
  private setProductName(products: Product[]): void {
    ...
    if (products && products.length > 0) {
      const foundProduct = products.find(p => p.id.toString() === prodVal);
      if (foundProduct) {
  ...
  async updateStatusByDeveloper(): Promise<void> {
    ...
    try {
      ...
      const errMsg = error?.error?.error || error?.error?.message || error?.message || 'Failed to update status.';
  ...
  selectTab(tab: 'developer' | 'customerChat' | 'timeline' | 'relations'): void {
    this.activeTab = tab;
  }
