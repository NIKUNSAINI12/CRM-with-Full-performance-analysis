...
export interface AuthUser {
  ...
  userNumber?:  string;
  detailViewType?: string; // 'SuperManager' or 'Executive'
}
...
export class AuthService {
  ...
  getToken(): string | null {
    ...
    // Fallback: try sessionStorage (e.g. after page refresh)
    const stored = sessionStorage.getItem(this.SESSION_KEY);
    if (stored) {
      try {
        const user: AuthUser = JSON.parse(stored);
  ...
  async refreshSession(): Promise<AuthUser | null> {
    try {
      const res = await firstValueFrom(
        this.http.get<AuthUser>(`${this.apiUrl}/auth/refresh-session`, { withCredentials: true })
      );
      this.storeSession(res);
      return res;
    } catch (e) {
      console.error('Failed to refresh session payload:', e);
      return null;
    }
  }
  ...
  getTicketDetailRoute(ticketId: number): string {
    ...
    // Check if preferred detail view type is set, or fall back to role default
    const isExecutiveDefault = user.role === 'Assignee' || user.role === 'Developer';
    const viewType = user.detailViewType || (isExecutiveDefault ? 'Executive' : 'SuperManager');
