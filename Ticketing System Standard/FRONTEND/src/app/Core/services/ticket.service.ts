...
export interface TicketService {
  ...
  getTicketDetails(ticketId: number): Promise<Ticket>;
  getAssignees(includeInactive?: boolean): Promise<Assignee[]>;
...
: Promise<TicketResponse>; // Changed to match the response type
  ...
  toggleUserActive(id: number, isActive: boolean): Promise<void>;
  getUserProducts(userId: number): Promise<number[]>;
  getUserAssignees(userId: number): Promise<number[]>;
  getTicketDeadline(ticketId: number): Promise<TicketDeadline | null>;
  getTicketDeadlinesBatch(ticketIds: number[]): Promise<TicketDeadline[]>;
  ...
  // --- Sprint Feature ---
  getSprints(pmId?: number): Promise<Sprint[]>;
  createSprint(sprint: Sprint): Promise<Sprint>;
  updateSprint(id: number, sprint: Sprint): Promise<Sprint>;
  deleteSprint(id: number): Promise<void>;
  startSprint(id: number): Promise<Sprint>;
  completeSprint(id: number, destinationSprintId?: number | null): Promise<Sprint>;
  assignTicketsToSprint(sprintId: number | null, ticketIds: string): Promise<void>;
  getSprintAnalytics(pmId?: number): Promise<SprintAnalytics>;
  updateTicketOrder(orders: { ticketId: number; order: number }[]): Promise<void>;
  getCompletedSprintsHistory(pmId?: number): Promise<CompletedSprintHistory[]>;
...
export interface Sprint {
  ...
  createdBy?: number;
  createdOn?: string;
  totalTicketsAtClose?: number;
  completedTicketsAtClose?: number;
  inProgressTicketsAtClose?: number;
  openTicketsAtClose?: number;
  onHoldTicketsAtClose?: number;
  reworkTicketsAtClose?: number;
}
...
export interface TicketCommunicationDto {
  ...
  channel: 'Customer' | 'Developer'|'Customer(Note)'|'Developer(Note)'; // <-- ADD THIS PROPERTY
}
...
export class DefaultTicketService implements TicketService {
  ...
  async getAllCustomers(): Promise<any[]> {
    return await firstValueFrom(
      this.http.get<any[]>(`${this.apiUrl}/Manager/all-customers`)
    );
  }
...
async getProductHierarchys(): Promise<ProductData[]> {
  ...
  async toggleUserActive(id: number, isActive: boolean): Promise<void> {
    await firstValueFrom(
      this.http.put<void>(`${this.apiUrl}/Manager/users/${id}/toggle-active?isActive=${isActive}`, {})
    );
  }
  ...
  async getUserAssignees(userId: number): Promise<number[]> {
    return await firstValueFrom(
        this.http.get<number[]>(`${this.apiUrl}/Manager/users/${userId}/assignees`)
    );
  }
 public async getTicketDeadline(ticketId: number): Promise<TicketDeadline | null> {
  ...
  // Batch fetch - single API call instead of N calls
  public async getTicketDeadlinesBatch(ticketIds: number[]): Promise<TicketDeadline[]> {
    if (!ticketIds || ticketIds.length === 0) return [];
    try {
      const response = await firstValueFrom(
        this.http.post<TicketDeadline[]>(`${this.apiUrl}/manager/tickets/deadlines/batch`, ticketIds)
      );
      return response ?? [];
    } catch (error) {
      console.error('Failed to batch-fetch deadlines:', error);
      return [];
    }
  }
...
async getDashboardStats(pmId: number): Promise<DashboardStats> {
  ...
  async getCustomerById(id: number): Promise<Customer> {
    // This assumes you have an endpoint like GET /api/Customers/{id}
    ...
    try {
      ...
      // Return a default object on failure to prevent the app from crashing
      return { id: id, name: 'Unknown Customer' };
    }
  }
  async getAssignees(includeInactive: boolean = false): Promise<Assignee[]> {
    try {
      // This endpoint gets the list of users who can be assigned a ticket.
      const url = includeInactive 
        ? `${this.apiBaseUrl}/Manager/assignees?includeInactive=true` 
        : `${this.apiBaseUrl}/Manager/assignees`;
      const assignees = await firstValueFrom(
        this.http.get<Assignee[]>(url)
      );
      return assignees;
    } catch (error) {
...
async updateTicketAsPm(ticketId: number, pmId: number, payload: PmTicketUpdatePayload): Promise<void> {
  ...
  async getSprints(pmId?: number): Promise<Sprint[]> {
    const url = pmId ? `${this.apiUrl}/sprints?pmId=${pmId}` : `${this.apiUrl}/sprints`;
    return firstValueFrom(this.http.get<Sprint[]>(url));
  }

  async createSprint(sprint: Sprint): Promise<Sprint> {
    return firstValueFrom(this.http.post<Sprint>(`${this.apiUrl}/sprints`, sprint));
  }

  async updateSprint(id: number, sprint: Sprint): Promise<Sprint> {
    return firstValueFrom(this.http.put<Sprint>(`${this.apiUrl}/sprints/${id}`, sprint));
  }

  async deleteSprint(id: number): Promise<void> {
    return firstValueFrom(this.http.delete<void>(`${this.apiUrl}/sprints/${id}`));
  }

  async startSprint(id: number): Promise<Sprint> {
    return firstValueFrom(this.http.post<Sprint>(`${this.apiUrl}/sprints/${id}/start`, {}));
  }

  async completeSprint(id: number, destinationSprintId?: number | null): Promise<Sprint> {
    return firstValueFrom(this.http.post<Sprint>(`${this.apiUrl}/sprints/${id}/complete`, { destinationSprintId: destinationSprintId ?? null }));
  }

  async assignTicketsToSprint(sprintId: number | null, ticketIds: string): Promise<void> {
    const url = sprintId 
      ? `${this.apiUrl}/sprints/${sprintId}/assign-tickets`
      : `${this.apiUrl}/sprints/0/remove-tickets`;
    return firstValueFrom(this.http.post<void>(url, { ticketIds }));
  }

  async getSprintAnalytics(pmId?: number): Promise<SprintAnalytics> {
    const url = pmId ? `${this.apiUrl}/sprints/analytics?pmId=${pmId}` : `${this.apiUrl}/sprints/analytics`;
    return firstValueFrom(this.http.get<SprintAnalytics>(url));
  }

  async updateTicketOrder(orders: { ticketId: number; order: number }[]): Promise<void> {
    await firstValueFrom(this.http.post<void>(`${this.apiUrl}/sprints/update-ticket-order`, { orders }));
  }

  async getCompletedSprintsHistory(pmId?: number): Promise<CompletedSprintHistory[]> {
    const url = pmId ? `${this.apiUrl}/sprints/completed-history?pmId=${pmId}` : `${this.apiUrl}/sprints/completed-history`;
    return firstValueFrom(this.http.get<CompletedSprintHistory[]>(url));
  }
  ...
  async getStatusWorkflows(): Promise<any[]> {
    return firstValueFrom(this.http.get<any[]>(`${this.apiUrl}/Roles/status-workflows`));
  }

  async getAssignmentWorkflows(): Promise<any[]> {
    return firstValueFrom(this.http.get<any[]>(`${this.apiUrl}/Roles/assignment-workflows`));
  }
  ...
  async saveUserPreferences(payload: any): Promise<any> {
    return firstValueFrom(
      this.http.post<any>(`${this.apiUrl}/UserPreferences`, payload)
    );
  }
}

export interface CompletedSprintHistory {
  sprint: Sprint;
  tickets: SprintTicketHistory[];
}

export interface SprintTicketHistory {
  historyId: number;
  sprintId: number;
  ticketId: number;
  ticketNumber: string;
  ...
  rolledToSprintId?: number | null;
  rolledToSprintName?: string | null;
}
