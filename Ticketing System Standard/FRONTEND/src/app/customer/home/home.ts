...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  public getDisplayStatus(ticket: Ticket): string {
    ...
    if (ticket.status === 'Work Done') return 'Work Done'; // <-- ADD THIS
    return 'Open'; // All other statuses are grouped as 'Open'
  }
  ...
  // --- UI Event Handlers ---
  public viewTicketDetails(event: any): void {
    const ticketId = event?.data?.id || event?.id;
    if (ticketId) {
