...
namespace ticketing_system_backend.Interface
{
  public interface IManager
  {
    ...
    Task<IEnumerable<TicketDeadline>> GetTicketDeadlinesBatchAsync(IEnumerable<int> ticketIds);
    ...
    Task<IEnumerable<int>> GetUserAssigneesAsync(int userId);
    ...
    // --- Assignee Management ---
    Task<IEnumerable<Assignee>> GetAllAssigneesAsync(int? pmId = null, bool includeInactive = false);
    ...
    Task<CustomTicketStatus> CreateCustomStatusAsync(CustomStatusRequest request);
    Task UpdateCustomStatusAsync(int id, CustomStatusRequest request);
    ...
    Task<PriorityMaster> CreatePriorityAsync(string priorityName, int tatHours);
