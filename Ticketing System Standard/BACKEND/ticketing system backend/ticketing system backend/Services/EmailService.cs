...
public class SmtpEmailService : IEmailService
{
  ...
  public SmtpEmailService(string host, int port, string fromAddress, string password)
  {
    _host = host;
    _port = port;
  ...
  public async Task SendEmailAsync(string toEmail, string subject, string body, string? threadMessageId = null, bool isThreadRoot = false)
  {
    ...
    {
      From    = new MailAddress(_fromAddress, "Empower Logics Support"),
      Subject = subject,
      Body    = body,
      IsBodyHtml = true,
    };
    ...
    */
    ...
    // Fallback: Write the HTML email to the Desktop for easy local testing when SMTP is blocked
    try
    {
      var sanitizedEmail = toEmail.Replace("@", "_").Replace(".", "_");
      var fileName = $"email_to_{sanitizedEmail}_{System.DateTime.Now:yyyyMMdd_HHmmss}.html";
      var desktopPath = @"C:\Users\USER\Desktop";
    ...
    {
      System.Console.WriteLine($"[EmailService] Failed to write fallback email file: {ex.Message}");
    }
